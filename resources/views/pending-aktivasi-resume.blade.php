    ...
    <div class="mx-auto flex min-h-full w-full max-w-xl flex-col justify-center text-center">
        ...
        <div class="card animate-fade-in-up border-white/20 bg-white/80 p-6 text-left shadow-2xl backdrop-blur md:p-7 dark:bg-slate-950/60">
            <form action="{{ route('register.guru.pending.resume') }}" method="POST">
                ...
                <div class="flex flex-col gap-4">
                    <div>
                        ...
                        <input
                            ...
                            required
                        >
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Boleh tulis tanpa gelar atau tanda baca, misalnya Siti Aisyah.</p>
                    </div>
                ...
                <button type="submit" class="btn-primary mt-5 w-full">Buka Halaman Pembayaran</button>
            </form>
            ...
            <div class="mt-5 rounded-xl border border-amber-100 bg-amber-50 p-4 text-sm text-amber-900">
                Halaman ini dipakai jika tab tertutup, session browser habis, atau Anda melanjutkan dari perangkat lain.
            </div>
        </div>

        <div class="mt-7">
            <a href="{{ route('register.guru.form') }}" class="text-sm font-bold text-primary hover:underline">Kembali ke form pendaftaran</a>
        </div>
    </div>
@endsection
