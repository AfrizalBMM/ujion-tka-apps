...
<div class="mb-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-textSecondary">Review detail pengerjaan siswa pada mapel {{ $session->mapelPaket->nama_label }}.</p>
        </div>
        <div class="rounded-2xl bg-white px-6 py-3 shadow-sm border border-slate-100 flex items-center gap-4">
            <div class="text-right">
                <div class="text-[10px] font-bold uppercase tracking-widest text-textSecondary">Skor Total</div>
...
<div class="mb-8 rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
    ...
    <div class="flex flex-wrap gap-3">
        ...
        @endphp
        <div class="flex h-10 w-10 items-center justify-center rounded-xl font-bold text-white shadow-sm {{ $ans ? ($isCorrect ? 'bg-emerald-500' : 'bg-rose-500') : 'bg-slate-300' }}" title="Nomor {{ $s->nomor_soal }}">
            {{ $s->nomor_soal }}
        </div>
        @endforeach
    </div>
...
<div class="space-y-6">
    ...
    @php
        ...
        $isCorrect = ($ans && $s->tipe_soal === 'pilihan_ganda' && $ans->jawaban_pg === $correctOption?->kode);
    @endphp
    <div class="rounded-[32px] border border-white/80 bg-white/80 p-8 shadow-card transition-all duration-300 hover:shadow-hover">
        <div class="mb-6 flex items-center justify-between">
            ...
            <span class="text-xs font-bold {{ $isCorrect ? 'text-emerald-600' : 'text-rose-600' }}">
                {{ $isCorrect ? 'BENAR (+'.$s->bobot.')' : 'SALAH (+0)' }}
            </span>
        </div>
        ...
        <div class="rounded-2xl bg-slate-50 p-6">
            ...
            <div class="space-y-3">
                ...
                @php
                    $studentChoice = $ans->jawaban_menjodohkan[$pair->id] ?? null;
                    $isPairCorrect = ($studentChoice == $pair->jawaban_benar);
                @endphp
                <div class="flex items-center justify-between rounded-xl bg-white p-3 shadow-sm border border-slate-100">
                    <div class="text-sm font-semibold text-slate-800">{{ $pair->pertanyaan_teks }}</div>
                    <div class="flex items-center gap-3">
                        <div class="text-sm border-b-2 {{ $isPairCorrect ? 'border-emerald-500' : 'border-rose-500' }} px-2 font-bold">{{ $studentChoice ?: 'Kosong' }}</div>
                        <div class="text-[10px] text-textSecondary">Kunci: {{ $pair->jawaban_benar }}</div>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="mt-8 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-5">
            <div class="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-indigo-700">
                <i class="fa-solid fa-lightbulb"></i> Pembahasan / Indikator
            </div>
