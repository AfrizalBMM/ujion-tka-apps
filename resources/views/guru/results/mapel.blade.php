    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <nav class="mb-4 flex items-center gap-2 text-sm font-semibold text-textSecondary">
                ...
                <a href="{{ route('guru.results.show', $exam->id) }}" class="hover:text-primary">{{ $exam->judul ?? $exam->nama }}</a>
                <i class="fa-solid fa-chevron-right text-[10px]"></i>
            ...
            <p class="mt-1 text-sm text-textSecondary">
                {{ ($isSurvey ?? false) ? 'Analisis distribusi respons, skor per dimensi, dan kategori profil siswa.' : 'Informasi analisis mendalam untuk pengerjaan ' . $mapel->nama_label . '.' }}
            </p>
        </div>
...
<div class="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <div class="stat-card">
        ...
        <div>
            <div class="metric-label">{{ ($isSurvey ?? false) ? 'Jumlah Responden' : 'Total Peserta' }}</div>
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="metric-label">{{ ($isSurvey ?? false) ? 'Indeks Respons' : 'Rata-rata Skor' }}</div>
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="metric-label">{{ ($isSurvey ?? false) ? 'Indeks Tertinggi' : 'Skor Tertinggi' }}</div>
    ...
    <div class="stat-card">
        ...
        <div>
            <div class="metric-label">{{ ($isSurvey ?? false) ? 'Indeks Terendah' : 'Skor Terendah' }}</div>
    ...
    <div class="lg:col-span-2">
        <div class="rounded-[32px] border border-white/80 bg-white/80 overflow-hidden shadow-card">
            <div class="border-b border-slate-100 px-6 py-5">
                <h3 class="text-lg font-bold text-slate-900">{{ ($isSurvey ?? false) ? 'Daftar Respons Siswa' : 'Daftar Hasil Peserta' }}</h3>
            </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-slate-50/50 text-[10px] font-bold uppercase tracking-[0.15em] text-textSecondary">
                            ...
                            <th class="px-6 py-4 text-center">{{ ($isSurvey ?? false) ? 'Indeks' : 'Skor' }}</th>
                            ...
                            <tr class="group hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center">
                                        <span class="rounded-xl px-3 py-1 text-sm font-black {{ $s->skor >= 70 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                            {{ number_format((float) $s->skor, 1) }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('guru.results.student', $s->id) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:bg-indigo-600 hover:text-white" title="Lihat Detail">
    ...
    <div class="space-y-6">
        @if($isSurvey ?? false)
            <div class="rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
                ...
                <div class="space-y-3">
                    @forelse(($surveyOverview['category_distribution'] ?? []) as $label => $count)
                    ...
                    @empty
                        <div class="text-sm text-textSecondary">Belum ada distribusi profil.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
                ...
                <div class="space-y-3">
                    @forelse(($surveyOverview['dimension_stats'] ?? []) as $dimension)
                        <div class="rounded-2xl bg-slate-50 p-4">
                            <div class="flex items-center justify-between gap-3">
                                <div class="font-semibold text-slate-900">{{ $dimension['dimensi'] }}</div>
                                <div class="text-sm font-black text-indigo-600">{{ number_format((float) $dimension['score_percent'], 1) }}</div>
                            </div>
                            <div class="mt-1 text-xs text-textSecondary">{{ $dimension['category'] }}</div>
                        </div>
                    @empty
                        <div class="text-sm text-textSecondary">Belum ada data dimensi.</div>
                    @endforelse
                </div>
            </div>
        @else
                    ...
                    @foreach($sessions->take(5) as $index => $s)
                        <div class="flex items-center gap-4 rounded-2xl bg-slate-50 p-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="text-[10px] text-textSecondary">{{ number_format((float) $s->skor, 1) }} Poin</div>
                            </div>
                        </div>
                    @endforeach
                </div>
...
@if($isSurvey ?? false)
    <div class="mt-8 rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
        ...
        <div class="space-y-4">
            @forelse(($surveyOverview['question_breakdown'] ?? []) as $question)
                <div class="rounded-2xl bg-slate-50 p-4">
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div class="font-semibold text-slate-900">Butir {{ $question['nomor'] }} · {{ $question['dimensi'] }}</div>
                        @if($question['subdimensi'])
                            <div class="text-xs text-textSecondary">{{ $question['subdimensi'] }}</div>
                        @endif
                    </div>
                    <div class="mt-3 grid gap-2 md:grid-cols-2">
                        @foreach($question['distribution'] as $option)
                            <div class="rounded-xl bg-white px-3 py-2 text-sm shadow-sm">
                                <div class="font-semibold text-slate-900">{{ $option['kode'] }} · {{ \Illuminate\Support\Str::limit(strip_tags($option['label']), 48) }}</div>
            ...
            @empty
                <div class="text-sm text-textSecondary">Belum ada distribusi jawaban.</div>
            @endforelse
        </div>
    </div>
@endif
