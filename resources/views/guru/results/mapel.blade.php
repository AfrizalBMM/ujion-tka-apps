    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <nav class="mb-4 flex items-center gap-2 text-sm font-semibold text-textSecondary">
                ...
                <a href="{{ route('guru.results.show', $exam->id) }}" class="hover:text-primary">{{ $exam->nama }}</a>
                <i class="fa-solid fa-chevron-right text-[10px]"></i>
                <span class="text-slate-900">{{ $mapel->nama_label }}</span>
            </nav>
            <h1 class="text-2xl font-bold text-slate-900">{{ $mapel->nama_label }}</h1>
            <p class="mt-1 text-sm text-textSecondary">Informasi analisis mendalam untuk pengerjaan {{ $mapel->nama_mapel }}.</p>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="group hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('guru.results.student', $s->id) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:bg-indigo-600 hover:text-white" title="Lihat Jawaban Per Butir">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-textSecondary">Belum ada siswa yang menyelesaikan mapel ini.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
