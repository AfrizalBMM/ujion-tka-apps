...
<div class="space-y-6">
        ...
        @foreach($paket->mapelPakets as $mapel)
            <article class="card">
                <div class="section-heading mb-4">
                    ...
                    <a href="{{ route('guru.soal.index', [$paket, $mapel]) }}" class="btn-primary px-4 py-2 text-xs">{{ $canManage ? 'Kelola' : 'Lihat Soal' }}</a>
                </div>
                @if($canManage)
                    <form class="grid gap-3 rounded-[24px] border border-slate-200/80 bg-slate-50/70 p-4 md:grid-cols-3 dark:border-slate-800 dark:bg-slate-900/60" method="POST" action="{{ route('guru.mapel.update', [$paket, $mapel]) }}">
                        @csrf
                        @method('PUT')
    ...
    <section class="card">
        <div class="section-heading mb-4">
            <div>
                ...
                <p class="section-description">
                    ...
                    <a href="{{ route('guru.exams') }}" class="font-semibold text-primary underline underline-offset-2">Simulasi Ujian</a>
        ...
        @if($paket->exams->isEmpty())
            <div class="flex items-center gap-3 rounded-2xl border border-amber-100 bg-amber-50/80 p-4 dark:border-amber-800/40 dark:bg-amber-900/20">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-amber-700 dark:text-amber-400">
                        Superadmin belum membuat atau mengaktifkan ujian dari paket soal ini. Hubungi superadmin untuk mendapatkan token.
                    </p>
                </div>
            </div>
        @else
            <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                @foreach($paket->exams as $exam)
                    <div class="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/60 p-4 dark:border-slate-700 dark:bg-slate-900/60">
                        <div>
                        ...
                        <div class="space-y-2">
                            ...
                            @else
                                @foreach($exam->examMapelTokens as $mt)
                                    <div class="flex items-center gap-2 rounded-xl border border-slate-200/50 bg-white/50 p-2 dark:border-slate-800/50 dark:bg-slate-950/30">
                                        ...
                                        <button
                                            type="button"
                                            id="copy-guru-paket-{{ $mt->id }}"
                                            onclick="guruPaketCopyMapelToken('{{ $mt->token }}', {{ $mt->id }})"
                                            title="Salin token"
                        ...
                        <a href="{{ route('guru.exams') }}"
                           class="btn-primary w-full py-2 text-center text-xs">
