...
<div class="space-y-6">
    <section class="page-hero">
        ...
        <div class="page-actions">
            <a href="{{ route('superadmin.soal.index', [$paket, $mapel]) }}" class="btn-secondary border-white/20 bg-white/10 text-white hover:bg-white/15 hover:text-white">Kembali ke Soal</a>
        </div>
    </section>
    ...
    <section class="grid gap-6 lg:grid-cols-12">
        <div class="lg:col-span-4">
            <div class="card lg:sticky lg:top-6">
                ...
                <form method="POST" action="{{ route('superadmin.teks-bacaan.store', [$paket, $mapel]) }}" class="space-y-4">
                    @csrf
                    <div class="input-group">
                        <label class="text-xs font-bold uppercase tracking-[0.18em] text-textSecondary">Judul (opsional)</label>
                        <input name="judul" class="input" value="{{ old('judul') }}" placeholder="Teks 1: Kisah Singkat">
                    </div>
                    <div class="input-group">
                        ...
                        <div class="rounded-[24px] border border-slate-200/80 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/60">
                            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                ...
                                <div class="flex gap-2 md:flex-col">
                                    ...
                                    <form method="POST" action="{{ route('superadmin.teks-bacaan.destroy', [$paket, $mapel, $bacaan]) }}">
                                        @csrf
                                        @method('DELETE')
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-edit-bacaan]').forEach((button) => {
        button.addEventListener('click', () => {
            ...
            form.action = "{{ route('superadmin.teks-bacaan.update', [$paket, $mapel, 'bacaan' => '__ID__']) }}".replace('__ID__', data.id);
