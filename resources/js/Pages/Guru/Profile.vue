...
const copyBtnClass = 'inline-flex shrink-0 items-center gap-1.5 rounded-xl border border-teal-200/70 bg-white/80 px-2.5 py-1 text-xs font-semibold text-teal-700 transition hover:border-teal-400 hover:bg-teal-50 dark:border-teal-500/30 dark:bg-slate-950/60 dark:text-teal-300 dark:hover:bg-teal-500/10';
...
const submitPassword = () => {
	form.post(route('guru.profile.password'), {
					...
					<div class="flex items-start gap-4 sm:gap-5">
						...
						<div class="space-y-3">
							...
							<div class="flex flex-wrap gap-2">
								<Link :href="route('guru.profile.edit')" class="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-teal-700 focus:outline-none focus:ring-4 focus:ring-teal-500/20 dark:bg-teal-500 dark:text-slate-950 dark:hover:bg-teal-400">
				...
				<div class="relative mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					<div class="rounded-2xl border border-white/70 bg-white/75 p-4 backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/55">
						<div class="flex items-center justify-between gap-2">
							...
							<button type="button" :class="copyBtnClass" data-copy-token :data-copy-text="user.email" @click="copyValue('email', user.email)">
								<i :class="copiedKey === 'email' ? (copiedSuccess ? 'fa-solid fa-check text-xs' : 'fa-solid fa-xmark text-xs') : 'fa-regular fa-copy text-xs'"></i>
								<span>{{ copiedKey === 'email' ? (copiedSuccess ? 'Token tersalin' : 'Gagal menyalin') : 'Salin' }}</span>
							</button>
						</div>
					...
					<div class="rounded-2xl border border-white/70 bg-white/75 p-4 backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/55">
						<div class="flex items-center justify-between gap-2">
							...
							<button v-if="user.no_wa" type="button" :class="copyBtnClass" data-copy-token :data-copy-text="user.no_wa" @click="copyValue('no_wa', user.no_wa)">
								<i :class="copiedKey === 'no_wa' ? (copiedSuccess ? 'fa-solid fa-check text-xs' : 'fa-solid fa-xmark text-xs') : 'fa-regular fa-copy text-xs'"></i>
								<span>{{ copiedKey === 'no_wa' ? (copiedSuccess ? 'Token tersalin' : 'Gagal menyalin') : 'Salin' }}</span>
							</button>
						</div>
					...
					<div class="rounded-2xl border border-teal-200/80 bg-teal-50/60 p-4 dark:border-teal-500/20 dark:bg-teal-500/5">
						<div class="flex items-center justify-between gap-2">
							...
							<button type="button" :class="copyBtnClass" data-copy-token :data-copy-text="user.access_token" @click="copyValue('access_token', user.access_token)">
								<i :class="copiedKey === 'access_token' ? (copiedSuccess ? 'fa-solid fa-check text-xs' : 'fa-solid fa-xmark text-xs') : 'fa-regular fa-copy text-xs'"></i>
								<span>{{ copiedKey === 'access_token' ? (copiedSuccess ? 'Token tersalin' : 'Gagal menyalin') : 'Salin' }}</span>
							</button>
						</div>
					...
					<div class="rounded-2xl border border-white/70 bg-white/75 p-4 backdrop-blur dark:border-slate-700/70 dark:bg-slate-900/55">
						...
						<template v-if="user.google_connected">
				...
				<section class="overflow-hidden rounded-[30px] border border-slate-200/70 bg-white shadow-[0_24px_70px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-slate-950/95">
					...
					<form class="space-y-4 px-6 py-6" @submit.prevent="submitPassword">
						<div class="space-y-2">
							...
							<input id="password" v-model="form.password" type="password" name="password" required minlength="8"
								:class="form.errors.password ? 'border-rose-300 bg-rose-50/50 dark:border-rose-500/40 dark:bg-rose-500/10' : 'border-slate-200 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-900/80'"
							...
							<p v-if="form.errors.password" class="text-sm text-rose-600 dark:text-rose-300">{{ form.errors.password }}</p>
						</div>
						<div class="space-y-2">
							<label for="password_confirmation" class="text-sm font-semibold text-slate-700 dark:text-slate-200">Konfirmasi Password</label>
							<input id="password_confirmation" v-model="form.password_confirmation" type="password" name="password_confirmation" required minlength="8"
