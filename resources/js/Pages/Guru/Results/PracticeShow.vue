...
<template>
	<Head title="Detail Latihan Materi — Analisis" />
			...
			<div class="flex flex-wrap items-center justify-between gap-4">
				<div>
					<nav class="mb-4 flex items-center gap-2 text-sm font-semibold text-textSecondary">
						<Link :href="route('guru.results.index', { tab: 'materi' })" class="hover:text-primary">Hasil Siswa</Link>
				...
				<Link :href="route('guru.results.index', { tab: 'materi' })" class="btn-secondary px-5 py-2.5 text-sm font-bold">
					<i class="fa-solid fa-arrow-left mr-2"></i> Kembali
				</Link>
			</div>
		</div>

		<div v-if="!token" class="empty-state py-12">
		...
		<template v-else>
			...
			<div class="mb-8 rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
				<div class="flex flex-wrap items-center justify-between gap-3">
					...
					<div class="flex flex-wrap items-center gap-2">
						...
						<span class="badge-info">{{ packageStats.length }} paket</span>
			...
			<div class="grid gap-8 lg:grid-cols-3">
							...
							<table class="w-full text-left">
								...
								<tbody class="divide-y divide-slate-100">
									<template v-if="rankings.length > 0">
										<tr v-for="s in rankings" :key="s.id" class="group transition-colors hover:bg-slate-50/50">
											<td class="px-6 py-4">
											...
											<td class="px-6 py-4 text-right">
												<Link :href="route('guru.results.practice.student', [material.id, s.id])" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-colors hover:bg-indigo-600 hover:text-white" title="Lihat Detail">
				...
				<div class="space-y-6">
					<div class="rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
						...
						<div class="space-y-3">
							<div v-for="pkg in packageStats" :key="pkg.paket_no" class="rounded-2xl bg-slate-50 p-4">
								<div class="flex items-center justify-between">
									<div class="font-bold text-slate-900">Paket {{ pkg.paket_no }}</div>
									<div class="text-sm font-black text-indigo-600">{{ Number(pkg.avg).toFixed(1) }}</div>
								</div>
					...
					<div class="rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
						...
						<div class="grid grid-cols-5 gap-2">
							<template v-if="questionStats.length > 0">
								<div v-for="(q, index) in questionStats" :key="index" class="group relative">
									<div class="flex h-10 w-full items-center justify-center rounded-xl font-bold text-white shadow-sm transition-transform hover:scale-110" :class="q.percent >= 75 ? 'bg-emerald-500' : (q.percent >= 50 ? 'bg-amber-500' : 'bg-rose-500')" :title="`${q.percent}% benar`">
										{{ index + 1 }}
									</div>
							...
							<div v-else class="col-span-5 text-sm text-textSecondary">Belum ada jawaban paket.</div>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="mt-8 rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card">
				...
				<div class="grid gap-4 md:grid-cols-2">
					<template v-if="telaahStats.length > 0">
						<div v-for="(q, index) in telaahStats" :key="index" class="rounded-2xl bg-slate-50 p-4">
							<div class="flex items-center justify-between gap-3">
								<div class="font-semibold text-slate-900">Telaah {{ index + 1 }}</div>
								<div class="rounded-xl px-3 py-1 text-sm font-black" :class="q.percent >= 75 ? 'bg-emerald-100 text-emerald-700' : (q.percent >= 50 ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700')">
									{{ q.percent }}%
								</div>
							</div>
							<div class="mt-2 text-sm text-textSecondary">{{ q.question_text }}</div>
