...
<template>
	<Head title="Hasil Latihan Materi — Analisis" />
	...
	<GuruLayout>
		<div class="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
			...
			<div>
				<Link :href="route('guru.results.index')" class="btn-secondary">Kembali</Link>
			</div>
		</div>

		<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
			<template v-if="tokens.length > 0">
				<div v-for="t in tokens" :key="t.material_id" class="group relative flex flex-col overflow-hidden rounded-[32px] border border-white/80 bg-white/80 p-6 shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-hover">
