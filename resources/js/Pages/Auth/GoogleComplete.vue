...
<template>
	<Head title="Lengkapi Data - Ujion TKA" />
		...
		<div class="mx-auto flex min-h-full w-full max-w-xl flex-col justify-center text-center">
			...
			<div class="card animate-fade-in-up border-white/20 bg-white/80 p-6 text-left shadow-2xl backdrop-blur md:p-7 dark:bg-slate-950/60">
				<div class="mb-5 flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
					<img v-if="google.avatar" :src="google.avatar" alt="Avatar" class="h-12 w-12 rounded-full object-cover">
					<div v-else class="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-primary text-white">
				...
				<form class="space-y-4" @submit.prevent="submit">
					<div>
						...
						<div class="ssd-wrap">
							...
							<button
								type="button"
								class="ssd-trigger flex w-full items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white py-3 pl-4 pr-4 text-left outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-950/40 dark:text-white"
							>
