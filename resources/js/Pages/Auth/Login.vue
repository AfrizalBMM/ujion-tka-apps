...
const submit = () => {
	form.post(route('login'), {
		onFinish: () => form.reset('access_token'),
	});
};
</script>

<template>
	<Head title="Masuk Guru - Ujion TKA" />
	...
	<GuestLayout fullscreen hide-footer>
		...
		<div v-if="Object.keys(form.errors).length > 0" class="mb-5">
				<div class="flex gap-3 rounded-2xl border border-red-100 bg-red-50 p-4 text-sm text-red-600 dark:border-red-900/40 dark:bg-red-950/40 dark:text-red-300">
			...
			<div class="card animate-fade-in-up border-white/20 bg-white/80 p-6 shadow-2xl backdrop-blur md:p-7 dark:bg-slate-950/60">
				<a
					...
					class="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white py-3 text-sm font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
				>
				...
				<div class="my-5 flex items-center gap-3 text-xs font-semibold uppercase tracking-wide text-slate-400">
					<span class="h-px flex-1 bg-slate-200 dark:bg-slate-700"></span>
					atau masuk dengan WhatsApp
					<span class="h-px flex-1 bg-slate-200 dark:bg-slate-700"></span>
				</div>
