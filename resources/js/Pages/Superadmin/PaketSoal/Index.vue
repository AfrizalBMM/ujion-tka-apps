...
const selectedJenjangCode = (jenjangs, filter) => jenjangs.find((j) => j.id == filter)?.kode ?? null;
		...
		<div class="space-y-6">
			<section class="page-hero">
				...
				<div class="page-actions">
					<Link :href="route('superadmin.paket-soal.create')" class="btn-primary">Paket Baru</Link>
				</div>
			</section>
				...
				<form class="grid gap-4 md:grid-cols-[1fr_1fr_auto]">
					<div class="input-group">
						...
						<div class="ssd-wrap mt-1">
							<input type="hidden" name="jenjang_id" :value="jenjangFilter ?? ''">
							<button type="button" class="ssd-trigger input text-sm flex items-center justify-between gap-2 w-full">
								<span class="ssd-label">{{ selectedJenjangCode(jenjangs, jenjangFilter) || 'Semua jenjang' }}</span>
					...
					<div class="input-group">
						...
						<input type="text" name="tahun_ajaran" class="input" :value="tahunAjaranFilter" placeholder="2025/2026">
					</div>
										...
										<div class="relative inline-block" data-dropdown>
											...
											<div class="absolute right-0 top-full z-50 mt-1.5 hidden w-44 rounded-2xl border border-border bg-white shadow-xl dark:bg-slate-900"
												...
												<div class="p-1.5 space-y-0.5">
													...
													<Link :href="route('superadmin.paket-soal.show', paket.id)"
													...
													<Link :href="route('superadmin.paket-soal.edit', paket.id)"
														  class="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800 transition">
														<i class="fa-solid fa-pen-to-square w-4 text-amber-500"></i>
														Edit
													</Link>

													<form method="POST" :action="route('superadmin.paket-soal.toggle', paket.id)">
													...
													<form method="POST" :action="route('superadmin.paket-soal.destroy', paket.id)">
														<input type="hidden" name="_token" :value="$page.props.csrf_token" />
