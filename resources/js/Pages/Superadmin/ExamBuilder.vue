...
const add = () => {
	state.questions.push({ material_id: '', tipe: 'PG', pertanyaan: '', opsi: [''], jawaban_benar: '', pembahasan: '', image: '' });
...
const addOpsi = () => {
	state.questions[state.current].opsi.push('');
};

const removeOpsi = (i) => {
	state.questions[state.current].opsi.splice(i, 1);
};
...
const fetchBankQuestions = () => {
	...
	const url = `${route('superadmin.exams.bank-questions', props.exam.id)}?q=${state.bankSearch}&page=${state.bankPage}`;
	fetch(url)
		.then((r) => r.json())
		.then((data) => {
			state.bankQuestions = [...state.bankQuestions, ...data.data];
			state.hasMoreBank = data.current_page < data.last_page;
...
const importFromBank = () => {
	if (confirm(`Import ${state.selectedBankIds.length} soal terpilih ke dalam builder? (Catatan: Perubahan yang belum disimpan akan hilang jika tidak hati-hati)`)) {
		...
		fetch(route('superadmin.exams.import-bank', props.exam.id), {
			...
			body: JSON.stringify({ global_question_ids: state.selectedBankIds }),
		}).then((r) => {
			if (r.ok) {
...
const save = () => {
	...
	fetch(route('superadmin.exams.builder.save', props.exam.id), {
		method: 'POST',
		headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': page.props.csrf_token },
			...
			<div class="flex flex-1 flex-col overflow-hidden lg:flex-row">
				<div class="max-h-[350px] bg-white p-4 overflow-y-auto border-b lg:max-h-none lg:w-1/4 lg:border-r lg:border-b-0 flex flex-col">
					<div class="mb-4 font-bold flex items-center justify-between">
						...
						<span class="text-xs text-muted">{{ state.questions.length }} soal</span>
					</div>
					...
					<div class="mt-8 pt-6 border-t border-gray-100">
						...
						<input v-model="state.bankSearch"
						...
						<div class="max-h-64 overflow-y-auto border rounded-xl bg-gray-50/50 p-2 space-y-2">
							...
							<div v-else-if="state.bankQuestions.length === 0" class="p-4 text-center text-xs text-muted">
							...
							<button v-if="state.hasMoreBank && !state.loadingBank"
									@click="loadMoreBank"
									class="w-full py-2 text-xs font-bold text-primary hover:bg-white rounded-lg transition">
								Muat lebih banyak...
							</button>
							<div v-if="state.loadingBank && state.bankPage > 1" class="py-2 text-center text-xs text-muted italic">
								Memuat...
							</div>
						</div>

						<div class="mt-3 flex items-center gap-2">
							<div class="text-xs text-muted flex-1">{{ state.selectedBankIds.length }} soal dipilih</div>
							<button @click="importFromBank"
									:disabled="state.selectedBankIds.length === 0 || state.loadingImport"
									class="btn-secondary text-xs py-1.5">
								<i v-if="state.loadingImport" class="fa-solid fa-circle-notch fa-spin mr-1"></i>
								Import
							</button>
						</div>
					</div>
						...
						<div v-if="state.questions.length">
							<div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
								...
								<div class="flex items-center gap-2">
									...
									<button type="button" class="btn-secondary px-4" @click="next" :disabled="state.current===state.questions.length-1">
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
								<div>
									...
									<div class="ssd-wrap mt-1">
										<input type="hidden" :value="state.questions[state.current].material_id" @change="state.questions[state.current].material_id = $event.target.value">
										<button type="button" class="ssd-trigger input text-sm flex items-center justify-between gap-2 w-full">
											<span class="ssd-label">{{ materials.find(m => m.id == state.questions[state.current].material_id)?.sub_unit || 'Pilih materi' }}</span>
								...
								<div>
									...
									<div class="ssd-wrap mt-1">
										<input type="hidden" :value="state.questions[state.current].tipe" @change="state.questions[state.current].tipe = $event.target.value">
										<button type="button" class="ssd-trigger input text-sm flex items-center justify-between gap-2 w-full">
											<span class="ssd-label">{{ {PG: 'Pilihan Ganda', Checklist: 'Checklist', Singkat: 'Jawaban Singkat'}[state.questions[state.current].tipe] || 'PG' }}</span>
											<i class="fa-solid fa-chevron-down text-[10px] text-muted flex-shrink-0 ssd-icon"></i>
										</button>
										<div class="ssd-panel">
											...
											<div class="ssd-list">
												...
												<div class="ssd-option" :class="{'ssd-selected': state.questions[state.current].tipe === 'Singkat'}" data-value="Singkat">Jawaban Singkat</div>
											</div>
										</div>
									</div>
								</div>
							...
							<div class="mb-4">
								...
								<textarea v-model="state.questions[state.current].pertanyaan" class="input w-full mt-1" rows="4" placeholder="Tuliskan teks pertanyaan di sini..."></textarea>
							</div>
							<div class="mb-4" v-if="['PG','Checklist'].includes(state.questions[state.current].tipe)">
							...
							<div class="mb-4">
								...
								<input v-model="state.questions[state.current].jawaban_benar" class="input w-full mt-1" placeholder="Contoh: A (untuk PG) atau teks jawaban (untuk Singkat)">
							</div>
							<div class="mb-4">
								<label class="text-xs font-bold uppercase tracking-wider text-muted">Pembahasan</label>
								<textarea v-model="state.questions[state.current].pembahasan" class="input w-full mt-1" rows="3" placeholder="Opsional: penjelasan jawaban..."></textarea>
							</div>
							<div class="mb-6">
							...
							<div class="flex items-center justify-between gap-4 sticky bottom-0 bg-white/80 backdrop-blur py-4 border-t mt-8 -mx-4 px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
								<button type="button" class="text-rose-500 font-bold text-sm hover:underline" @click="remove(state.current)" v-if="state.questions.length>1">
									<i class="fa-solid fa-trash mr-1"></i> Hapus Soal
								</button>
								<div class="flex-1"></div>
								<button type="submit" class="btn-primary px-8 py-3 shadow-lg shadow-primary/20" :disabled="state.loadingSave">
									<i v-if="state.loadingSave" class="fa-solid fa-circle-notch fa-spin mr-2"></i>
